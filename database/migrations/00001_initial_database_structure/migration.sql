-- Migration: 00001_initial_database_structure
-- Migration Flow Observatory
--
-- Esta migration foi escrita manualmente (não gerada por `prisma migrate dev`,
-- já que o ambiente de implementação não teve acesso a um banco PostgreSQL
-- real). Ela é funcionalmente equivalente ao SQL que `prisma migrate dev`
-- geraria a partir de `schema.prisma`, com um acréscimo manual necessário:
-- a extensão PostGIS e o índice espacial GiST, que o Prisma Migrate não
-- gera automaticamente para colunas `Unsupported`.
--
-- Para aplicar: psql "$DATABASE_URL" -f database/migrations/00001_initial_database_structure/migration.sql
-- Ou, quando o ambiente tiver rede: npx prisma migrate deploy --schema=database/prisma/schema.prisma
...
-- Índice espacial GiST — essencial para performance de consultas
-- PostGIS (ST_DWithin, ST_Distance, proximidade, clustering). Não é
-- gerado pelo Prisma Migrate para colunas Unsupported; ver docs/postgis.md.
CREATE INDEX "location_geometry_gist_idx" ON "location" USING GIST ("geometry");

-- ──────────────────────────────────────────────────────────────
-- TABELA: migration_route
-- ──────────────────────────────────────────────────────────────
...
-- ──────────────────────────────────────────────────────────────
-- TABELA: app_user (reservada para autenticação futura)
-- ──────────────────────────────────────────────────────────────
